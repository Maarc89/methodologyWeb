...
class UserAssessment(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="user_assessments"
    )
    ...
    # Snapshot fields to keep username/email at creation time and to allow anonymization
    ...
    def save(self, *args, **kwargs):
        # On first save, populate snapshot fields from the linked user (if available)
        if not self.pk:
            if self.user:
                try:
                    ...
                    # defensive: ensure save proceeds even if user access fails
